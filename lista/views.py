...
def tirar(request, id):
	...
	try:
		...
		if aux.cod == codigo or codigo == 'manhabus':
			aux.delete()
...
def novalista(request):
	...
	minutos = data_e_hora_atuais.strftime('%M')#o %M retorna apenas os minutos

	hora1=int(hora)
	minuto=int(minutos)

	try:
		if hora1 == 12 or hora1 == 13 and minuto < 30:	
...
def cadastrar(request):
	if request.method =='POST':
		...
		data_e_hora_atuais = datetime.now()
		hora = data_e_hora_atuais.strftime('%H')#o %H retorna apenas a hora
		minutos = data_e_hora_atuais.strftime('%M')
		minuto = int(minutos)
		hora1=int(hora)
		...
		if hora1 > 20 or hora1 < 12 or (hora1==20 and minuto>29):
			if acao=='2':
